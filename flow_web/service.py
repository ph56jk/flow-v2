...
class FlowWebService:
    ...
    def _reset_ready_trello_outputs_sync(self, key: str, token: str, board_id: str, list_ids: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": (
                f"Đã reset {len(reset_cards)} card trong {scope_label}, xóa {deleted} ảnh output. "
    ...
    def _flow_operator_steps(
        ...
        source_detail = (
            ...
            if ready_candidates
            else f"Chỉ lấy attachment ảnh từ card ở {source_scope} hoặc card đã ghim rõ ràng."
        )
        return [
            {
...
"""
            ...
            return [
                {
                    ...
                    "brief": (
                        ...
                        "Only if the source image visibly contains an embroidered/personalized name, use a different plausible name on each fabric color option while keeping the same lettering style; otherwise keep every color option nameless."
                    ),
                    "must_include": "multiple pastel fabric colors, different personalized names only if the source has an embroidered name, white neutral daylight, same pillow embroidery style, no tag or label",
                },
                {
                    ...
                    "brief": (
                        ...
                        "Only if the source image has a visible embroidered/personalized name, assign a different embroidered name to each colorway; otherwise do not add names."
                    ),
                    "must_include": "crib or nursery bed, three pastel fabric variants, different visible names only if source has a name, source motif/name style preserved, clean white light",
                },
                {
                    ...
                    "brief": (
                        ...
                        "Only if the source image visibly has an embroidered name, show different embroidered sample names on the product variants; never add tag cards or text overlays."
                    ),
                    "must_include": "fabric swatches, pastel palette, different personalized names only if source has a name, embroidery thread, pillow texture, no tags",
                },
                {
                    ...
                    "brief": (
                        ...
                        "Only if the source image visibly has an embroidered/personalized name, each color option should carry a different embroidered name; otherwise all options must remain nameless."
                    ),
                    "must_include": "four color options, pastel fabric variety, different embroidered names only if source has a name, exact handmade style, white balanced light",
                },
            ]
        return [
            {
            ...
            },
            {
                ...
                "brief": (
                    ...
                    "Only if the source image visibly has a personalized embroidered name, include different embroidered sample names on the product variants rather than on separate labels; otherwise keep variants nameless."
                ),
                "must_include": "fabric swatches, pastel palette, different embroidered sample names only if source has a name, material texture, no tags",
            },
            {
                "label": "Color option display",
                "brief": (
        ...
        return [
            {
                ...
                "brief": (
                    ...
                    "but only if the source image visibly contains an embroidered/personalized name, use different embroidered names such as Emma, Olivia, Arthur, and Harper. "
                    "If the source has no embroidered name, show multiple same-style hoop variants without any names. The main variation is personalization/name when allowed, not fabric color."
                ),
                "must_include": "multiple embroidery hoops, different embroidered names only if source has a name, same hoop style, no colorway focus, no tags",
            },
            {
                "label": "Nursery shelf name variants",
                "brief": (
                    "Bright white nursery shelf or wall display with several personalized embroidery hoops from the same product family; each may carry a different name only when the source image visibly has an embroidered/personalized name. "
                    "If the source has no name, keep the hoops nameless. "
                ...
                "must_include": "nursery shelf or wall, different names only if source has a name, consistent embroidery hoop design, clean white light",
            },
            {
                "label": "Name customization flat lay",
                "brief": (
                    "Overhead craft flat lay showing the hoop with thread, needle, and small fabric/thread samples while presenting different embroidered name examples on actual hoops only if the source image visibly has an embroidered name. "
                    "If the source has no name, present same-style hoop options without names. "
                    "Use the same product design language and avoid separate text cards, tags, or labels."
                ),
                "must_include": "different name examples only if source has a name, craft props, thread texture, no tags or text overlays",
            },
            {
                "label": "Four personalized hoop options",
                "brief": (
                    "Ecommerce display with four real embroidery hoop variants arranged naturally; use different stitched names only if the source image visibly contains an embroidered/personalized name. "
                    "If the source has no name, keep every hoop nameless while preserving the same motif placement, hoop frame, fabric base, and handmade texture."
                ),
                "must_include": "four personalized name options only if source has a name, same hoop product family, clean white daylight, no fabric colorway emphasis",
            },
        ]
    ...
    async def _wait_for_new_project_images(
        ...
        *,
        ...
        while time.monotonic() < deadline:
            ...
            if len(best) >= target:
                return best[:target]
            if best and first_seen_at and time.monotonic() - first_seen_at >= settle_window:
                return best[:target]
            await asyncio.sleep(1.0)
        ...
        *,
        ...
        try:
            ...
            if job_id:
                await self.store.append_log(
                    job_id,
                    f"Fallback UI Flow: chưa chỉnh được model/số lượng trên UI ({humanize_flow_error(str(settings_exc))[:120]}).",
                )
        ...
        if flow_agent_enabled:
            ...
            if job_id:
                await self.store.append_log(
                    job_id,
                    (
                        f"Fallback UI Flow: đã mở/dùng panel Tác nhân ({panel_detail[:160]})."
                        if panel_ok
                        else f"Fallback UI Flow: chưa xác nhận được panel Tác nhân ({panel_detail[:160]})."
                    ),
                )
            if needs_prompt:
                raise RuntimeError(
                    "Tác nhân Flow đã mở panel nhưng app chưa gửi được lệnh trong panel Agent. "
                    "App đã dừng trước khi chờ ảnh để tránh chạy sai."
                )
        ...
        if not self._flow_image_call_uses_selected_image(
            result,
            reference_media_name,
            resolved_workflow_id,
            allow_any_image_input=attached_local_source,
        ):
            raise RuntimeError(
        ...
        if not images:
            ...
            if job_id:
                await self.store.append_log(
                    job_id,
                    f"Fallback UI Flow: response đầu có {len(images)}/{target_count} ảnh; app chờ project render thêm trước khi chạy lượt mới.",
                )
            try:
                project_images = await self._wait_for_new_project_images(
                    client,
                    known_media_before_submit,
                    prompt=prompt,
                    target_count=target_count,
                ...
                for image in [*images, *project_images]:
                    key = (
                        str(getattr(image, "media_name", "") or "").strip()
                        or str(getattr(image, "fife_url", "") or "").strip()
                        or str(getattr(image, "workflow_id", "") or "").strip()
                    )
                ...
                if job_id:
                    await self.store.append_log(
                        job_id,
                        f"Fallback UI Flow: sau khi chờ project có {len(images)}/{target_count} ảnh.",
                    )
            except Exception as project_exc:
                if job_id:
                    await self.store.append_log(
                        job_id,
                        f"Fallback UI Flow: chưa thấy thêm ảnh sau khi chờ project ({str(project_exc)[:220]}).",
                    )
    ...
    async def _ensure_flow_agent_panel_submitted(
        ...
        *,
        ...
        last_detail = "agent panel not visible"
        while asyncio.get_running_loop().time() < deadline:
            state = await self._flow_agent_panel_state(page, prompt)
            if state.get("visible"):
                ...
                filled, fill_detail = await self._fill_flow_agent_panel_instruction(page, prompt)
                if not filled:
                    return False, f"{detail}; {fill_detail}", True
                sent, send_detail = await self._click_flow_agent_panel_send(page)
                if not sent:
                    return False, f"{detail}; prompt filled but send failed: {send_detail}", True
                await asyncio.sleep(1.5)
                return True, f"{detail}; sent prompt in panel via {send_detail}", False
            last_detail = str(state.get("detail") or last_detail)
            await asyncio.sleep(0.5)
        return False, last_detail, False

    async def _flow_agent_panel_state(self, page: Any, prompt: str) -> Dict[str, Any]:
        prompt = str(prompt or "").strip()
        prompt_probe = prompt[:80]
        try:
            result = await page.evaluate(
                """
                ({ promptProbe }) => {
                  ...
                  const panels = [...document.querySelectorAll('aside, dialog, section, div[role="dialog"], div')]
                    ...
                    .map((el) => {
                      ...
                      const looksAgent = /Phiên\\s+không\\s+có\\s+tiêu\\s+đề|Phien\\s+khong\\s+co\\s+tieu\\s+de|Chào|Chao|Bạn\\s+muốn\\s+làm\\s+gì|Ban\\s+muon\\s+lam\\s+gi|Cùng\\s+tôi\\s+lên\\s+ý\\s+tưởng|Cung\\s+toi\\s+len\\s+y\\s+tuong/i.test(text);
                      const sidePanel = rect.width >= 280 && rect.height >= window.innerHeight * 0.45 && rect.left >= window.innerWidth * 0.35;
                      const score = (looksAgent ? 1000 : 0) + (sidePanel ? 300 : 0) + rect.height / 10;
                      return { el, rect, text, score, looksAgent, sidePanel };
                    })
                    .filter((item) => item.looksAgent || item.sidePanel)
                    .sort((a, b) => b.score - a.score);
                  const panel = panels[0];
                  if (!panel) return { visible: false, detail: 'agent panel not visible' };
                  const textboxes = [...panel.el.querySelectorAll('textarea, input[type="text"], [contenteditable="true"], [role="textbox"]')]
                    .filter((el) => {
                      if (!visible(el)) return false;
                      const label = labelFor(el);
                      const rect = el.getBoundingClientRect();
                      return rect.width >= 160 && rect.height >= 18 && /Bạn\\s*muốn\\s*tạo\\s*gì|Ban\\s*muon\\s*tao\\s*gi|What\\s+do\\s+you|prompt|create/i.test(label + ' ' + (el.textContent || ''));
                    });
                ...
                """,
                {"promptProbe": prompt_probe},
            )
        except Exception as exc:
            return {"visible": False, "detail": humanize_flow_error(str(exc))}
        return result if isinstance(result, dict) else {"visible": False, "detail": "agent panel state unavailable"}

    async def _fill_flow_agent_panel_instruction(self, page: Any, prompt: str) -> tuple[bool, str]:
        prompt = str(prompt or "").strip()
        if not prompt:
                ...
                () => {
                  ...
                  const candidates = [...document.querySelectorAll('textarea, input[type="text"], [contenteditable="true"], [role="textbox"]')]
                    ...
                    .map((el) => {
                      ...
                      const score = (inRightPanel ? 1000 : 0) + (agentish ? 600 : 0) + rect.bottom / 10;
                      return { el, rect, label, score };
                    })
                    .sort((a, b) => b.score - a.score);
                  const target = candidates[0];
                  if (!target) return { ok: false, detail: 'no agent panel textbox' };
                  target.el.scrollIntoView({ block: 'center', inline: 'center' });
                  const rect = target.el.getBoundingClientRect();
                  return {
                    ok: true,
                    ...
                    y: rect.top + rect.height / 2,
                    detail: target.label || target.el.tagName,
                  };
                }
                """
            )
        ...
        if not isinstance(result, dict) or not result.get("ok"):
            return False, str((result or {}).get("detail") or "no agent panel textbox") if isinstance(result, dict) else "no agent panel textbox"
        try:
    ...
    async def _click_flow_agent_panel_send(self, page: Any) -> tuple[bool, str]:
        ...
        try:
            await page.mouse.click(float(result.get("x")), float(result.get("y")))
            await asyncio.sleep(0.8)
            return True, str(result.get("detail") or "send")
        except Exception as exc:
            return False, humanize_flow_error(str(exc))
    ...
    def _flow_image_call_uses_selected_image(
        self,
        call: Any,
        reference_media_name: str,
        workflow_id: str,
        *,
        allow_any_image_input: bool = False,
    ) -> bool:
        ...
        for item in requests:
            ...
            if allow_any_image_input and image_inputs:
                return True
