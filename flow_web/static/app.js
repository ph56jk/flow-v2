...
// Một bản chụp duy nhất, chụp ngay trước khi Reset, để nút Hoàn tác lấy lại.
const AUTOMATION_UNDO_STORAGE_KEY = "flow-web-automation-dashboard-undo-v1";
...
const AUTOMATION_STEP_ORDER = ["source", "erp_source", "normalize", "flow", "review_hold", "log"];
...
function normalizeAutomationModules(parsed = {}) {
  ...
  // 10/9: bỏ hẳn bước xoá watermark; graph cũ còn nút này thì lọc đi.
  modules = modules.filter((module) => module.type !== "telegram" && module.type !== "watermark");
...
function normalizeERPState(value = {}) {
  ...
  return {
    ...
    sku_sheet_url: String(payload.sku_sheet_url || ""),
    sku_sheet_source: ["state", "env"].includes(payload.sku_sheet_source)
      ? payload.sku_sheet_source
      : "",
...
function normalizeIntegrationState(value = {}) {
  ...
  return {
    gemini: {
      ...
      credentials_source: String(payload.gemini?.credentials_source || ""),
      env_still_set: Boolean(payload.gemini?.env_still_set),
      env_var: String(payload.gemini?.env_var || ""),
...
const elements = {
  ...
  automationERPSkuSheetInput: document.querySelector("#automationERPSkuSheetInput"),
...
function showMessage(message, tone = "neutral", action = null) {
  if (!message) {
  ...
  // Nút hành động sống cùng thông báo; thông báo kế tiếp thay nó đi.
  if (action?.label && typeof action.onClick === "function") {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "message-bar-action";
    button.textContent = action.label;
    button.addEventListener("click", action.onClick);
    elements.messageBar.append(" ", button);
  }
}
...
function renderAutomationInspector(stats) {
  ...
  if (
    elements.automationERPSkuSheetInput &&
    document.activeElement !== elements.automationERPSkuSheetInput
  ) {
    elements.automationERPSkuSheetInput.value = state.erp?.sku_sheet_url || "";
  }
...
function saveAutomationUndoSnapshot(config) {
  try {
    window.localStorage?.setItem(AUTOMATION_UNDO_STORAGE_KEY, JSON.stringify(config));
  } catch (error) {
    // Không có localStorage thì không có gì để hoàn tác; nút Hoàn tác sẽ nói vậy.
  }
}

function undoAutomationReset() {
  let raw = "";
  try {
    raw = window.localStorage?.getItem(AUTOMATION_UNDO_STORAGE_KEY) || "";
  } catch (error) {
    raw = "";
  }
  if (!raw) {
    showMessage("Không còn bản chụp nào trước lần reset để hoàn tác.", "error");
    return;
  }
  try {
    state.automation = normalizeAutomationConfig(JSON.parse(raw));
  } catch (error) {
    showMessage("Bản chụp trước lần reset bị hỏng, không hoàn tác được.", "error");
    return;
  }
  saveAutomationConfig(state.automation);
  renderAll();
  showMessage("Đã hoàn tác: phần custom trở lại như trước lần reset.", "success");
}

function resetAutomationConfig() {
  const ok = window.confirm(
    "Reset phần custom về mặc định?\n\nToàn bộ graph automation, cài đặt module và tuỳ biến giao diện đang có sẽ bị xoá. Muốn giữ bản này thì bấm Huỷ rồi Export trước. Sau khi reset, thông báo có nút Hoàn tác để lấy lại bản ngay trước đó.",
  );
  if (!ok) {
    return;
  }
  saveAutomationUndoSnapshot(state.automation);
  state.automation = defaultAutomationConfig();
  saveAutomationConfig(state.automation);
  renderAll();
  showMessage("Đã reset phần custom về mặc định.", "success", {
    label: "Hoàn tác",
    onClick: undoAutomationReset,
  });
}
...
async function saveERPConfig({ clearCredentials = false } = {}) {
  ...
  const payload = {
    ...
    sku_sheet_url: elements.automationERPSkuSheetInput?.value?.trim() || "",
    // Xoá trắng ô rồi bấm Lưu là *cố ý* bỏ link, khác với màn hình chưa có ô
    // ấy gửi lên một payload thiếu khoá — bên server, thiếu khoá nghĩa là giữ
    // nguyên. Nói thẳng ra đây để hai ý đó không lẫn vào nhau.
    clear_sku_sheet_url: Boolean(
      elements.automationERPSkuSheetInput && !elements.automationERPSkuSheetInput.value.trim()
    ),
...
async function runERPIdeaBatch() {
  ...
  try {
    ...
    setIdeaBatchStatus(
      queued.length
        ? `${queued.length} idea đang chạy`
        : (reason ? "Chưa chạy" : "0 idea đang chạy"),
      queued.length ? "ready" : "pending"
    );
    const createdNote = created.length ? `; đã tách ${created.length} thẻ con` : "";
    ...
    if (queued.length) {
      ...
      showMessage(
        `Không tạo được ảnh: ${reason}${createdNote}${skippedNote}.`,
        "error",
      );
    } else {
      showMessage(`Không có idea nào cần chạy${createdNote}${skippedNote}.`, "error");
    }
    await loadState({ silent: true });
  } catch (error) {
...
async function saveIntegrationConfig({ clearSecrets = false } = {}) {
  ...
  try {
    ...
    if (clearSecrets) {
      const gemini = state.integrations?.gemini || {};
      if (gemini.env_still_set) {
        showMessage(
          `Đã xoá khoá trong app. Nhưng máy này còn ${gemini.env_var || "GEMINI_API_KEY"} trong .env.local nên app vẫn gọi được Gemini — xoá nốt trong file nếu muốn dừng hẳn.`,
          "warning",
        );
      } else {
        showMessage("Đã xoá khoá Gemini trong app. Các field không nhạy cảm vẫn giữ lại.", "success");
      }
    } else {
...
function confirmThenClearGeminiKey() {
  const gemini = state.integrations?.gemini || {};
  const envNote = gemini.env_var
    ? `\n\nLưu ý: máy này đang có ${gemini.env_var} trong .env.local, nên app vẫn gọi được Gemini sau khi xoá — muốn dừng hẳn thì xoá nốt trong file.`
    : "";
  const ok = window.confirm(`Xoá khoá Gemini đã lưu trong app?\n\nCác field không nhạy cảm (model, Playwright path) vẫn giữ lại.${envNote}`);
  if (!ok) {
    return;
  }
  void saveIntegrationConfig({ clearSecrets: true });
}

elements.automationEnvSaveButton?.addEventListener("click", () => saveIntegrationConfig());
elements.automationEnvClearButton?.addEventListener("click", confirmThenClearGeminiKey);
