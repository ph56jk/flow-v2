...
## Overview
Flow v2 needs to become an automation app where the local assistant only operates tools and workflow steps, while Google Flow's own Agent writes the final image prompts and starts generation. The production path is: choose the correct ERP card image, send the product/request context to Flow Agent, generate 4 images by default, auto-approve Flow Agent's internal confirmation, show results on the dashboard for human review, then archive approved images back to the same ERP card.
...
- Preserve dashboard human approval before writing final images back to ERP.
...
- A real or safe smoke run proving selected ERP image context reaches Flow Agent and returns artifacts or a clear recoverable error.
...
**Description:** As an operator, I want the main automation path to work without Google Sheet prompts, so that ERP plus Flow Agent is enough to create product images.

**Acceptance Criteria:**
- [ ] The default Auto ERP run does not fail because no Google Sheet prompt matches.
...
- [ ] Existing Sheet status does not block ERP image selection and Flow Agent execution.
...
- [ ] Assistant actions can search ERP, choose a card/attachment, set module values, and start the automation.
...
**Description:** As an operator, I want the app to talk to Google Flow Agent, attach the selected ERP reference, and approve Flow Agent's confirmation automatically, so that long generation runs do not stall.

**Acceptance Criteria:**
- [ ] The runner opens the configured Flow project and enables Agent mode when available.
- [ ] The runner sends product analysis instructions, selected ERP image context, and requested image count to Flow Agent.
...
### US-005: Dashboard Review and Same-Card ERP Archive
**Description:** As an operator, I want generated images reviewed on the dashboard before archiving, so that only approved outputs return to the original ERP card.

**Acceptance Criteria:**
- [ ] Flow Agent approval is automatic, but final image approval on the dashboard remains human-gated.
- [ ] Approved dashboard images are recorded back to the same source ERP card.
- [ ] Rejected or pending images are not archived to ERP.
- [ ] The history view shows source card, selected attachment, Flow artifacts, dashboard status, and archive result.
...
- FR-3: The system must not require a Google Sheet prompt for the primary Auto ERP path.
...
- FR-9: Dashboard final review must remain required before ERP archive.
- FR-10: Approved images must upload to the same source ERP card.
- FR-11: UI actions for selected ERP card/image must actually start the run.
...
- Do not auto-approve dashboard final review.
- Do not remove optional Google Sheet features entirely unless a later request asks for it.
- Do not expose ERP, Google, or Flow credentials in code, logs, or Git history.
...
- Reuse existing helpers such as Flow Agent mode switching, auto approval, project image scanning, ERP candidate rendering, and dashboard review queues.
...
## Success Metrics
- From a valid ERP card image, the user can start an Agent-based run and receive 4 generated images.
- No Google Sheet prompt match is needed for that run.
- Flow Agent confirmation does not block the run.
- Dashboard receives reviewable results.
- Approved results return to the same ERP card.
