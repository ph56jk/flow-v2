...
$excludedEnvKeys = @(
    "ERP_PROJECT_ID", "ERP_TASK_ID", "ERP_STATUS_ID", "FLOW_PUBLIC_URL",
...
$workerEnvLines += @(
    "", "ERP_PROJECT_ID=$workerProjectId", "ERP_TASK_ID=", "ERP_STATUS_ID=$workerStatus",
...
if (-not (Test-Path -LiteralPath $statePath)) {
    ...
    $state = [ordered]@{
        ...
        erp_config = [ordered]@{
            api_key = ""; api_secret = ""; base_url = "https://erp.havigroup.llc"; project_id = $workerProjectId; task_id = ""; status = $workerStatus; updated_at = ""
        }
