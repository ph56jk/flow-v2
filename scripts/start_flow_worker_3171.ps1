...
& (Join-Path $PSScriptRoot "setup_flow_worker_3171.ps1")
...
if ([string]::IsNullOrWhiteSpace($profileDirs) -or [string]::IsNullOrWhiteSpace($profileProjects)) {
    throw "Worker 3171 chua co Flow account. Hay cau hinh profile va project trong .env.worker-3171.local."
}
...
if ($null -ne $health) {
    ...
    Write-Host "Worker 3171 dang chay san tai $url"
...
do {
    ...
    if ($null -ne $health -and [string]$health.instance -eq $workerName) {
        Write-Host "Worker 3171 da chay tai $url - watchdog PID $($watchdogProcess.Id)."
        if (-not $NoOpenBrowser) {
            Start-Process $url
        }
        exit 0
...
throw "Worker 3171 khong khoi dong trong 60 giay. Xem logs\flow-web-3171-watchdog.log."
